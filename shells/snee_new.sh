...
rm -Rf ${username}*
cd ${SNEEROOT}/etc/
cp ${SNEEROOT}/etc/inqp.snee.properties ${SNEEROOT}/etc/${username}.wsn-dd.snee.properties
chmod 777 ${SNEEROOT}/etc/${username}.wsn-dd.snee.properties
sed -i "57s/.*/logical_schema = etc\/${username}_wsn-dd-logical-schema.xml/" ${username}.wsn-dd.snee.properties
sed -i "60s/.*/physical_schema = etc\/${username}_wsn-dd-physical-schema.xml/" ${username}.wsn-dd.snee.properties
sed -i "15s/.*/compiler.output_root_dir = etc\/${username}_output/" ${username}.wsn-dd.snee.properties
